...
export default function TrackingClient({ initialEvents, initialError, armadaOptions }: Props) {
  const [isPending, startTransition] = useTransition();
  const [modalOpen, setModalOpen] = useState(false);
  const [message, setMessage] = useState<string | null>(null);

  const [armadaLabel, setArmadaLabel] = useState("");
  const [location, setLocation] = useState("");
  const [note, setNote] = useState("");

  const events = useMemo(() => initialEvents ?? [], [initialEvents]);
  const options = useMemo(() => armadaOptions ?? [], [armadaOptions]);
  ...
  function submitCreate() {
    ...
    startTransition(async () => {
      const res = await createTrackingEvent({ armadaLabel, location, note });
      if (!res.ok) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-black tracking-tight text-slate-900">Live Tracking Armada</h1>
          <p className="mt-1 text-sm text-slate-500">Catat lokasi/status armada secara manual (sementara).</p>
        </div>
        <button
          type="button"
      ...
      {(initialError || message) && (
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {initialError?.includes("armada_tracking_events") && (
            <p className="mt-2 text-xs text-slate-500">
              Pastikan tabel <span className="font-mono">armada_tracking_events</span> ada di Supabase.
            </p>
          )}
        </div>
      )}
      ...
      <div className="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          {events.length === 0 ? (
            <div className="px-6 py-8 text-sm text-slate-500">Belum ada update.</div>
          ) : (
            events.map((e) => (
              <div key={e.id} className="px-6 py-5">
                <div className="flex items-center justify-between gap-4">
                  <p className="font-bold text-slate-900">{e.armada_label ?? "-"}</p>
                  <p className="text-xs text-slate-500">
                    {e.timestamp ? new Date(e.timestamp).toLocaleString("id-ID") : "-"}
                  </p>
                </div>
                <p className="mt-1 text-sm text-slate-600">
                  <span className="font-bold text-slate-900">[{e.location ?? "-"}]</span> {e.note ?? "-"}
                </p>
              </div>
            ))
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-lg rounded-3xl bg-white shadow-xl border border-slate-200">
            <div className="flex items-start justify-between border-b border-slate-100 px-6 py-4">
              <div>
                <h3 className="text-lg font-black text-slate-900">Tambah Update</h3>
                <p className="mt-0.5 text-xs text-slate-500">Update posisi/kondisi armada.</p>
              </div>
              <button
                type="button"
                onClick={closeModal}
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">Armada</label>
                {options.length > 0 ? (
                  <select
                    value={armadaLabel}
                    onChange={(e) => setArmadaLabel(e.target.value)}
                ...
                {options.length === 0 ? (
                  <p className="mt-2 text-xs text-slate-500">Opsional: buat armada dulu di menu Armada supaya bisa pilih dari dropdown.</p>
                ) : null}
              </div>
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-slate-400"
                  placeholder="Contoh: Rest Area KM 102"
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  placeholder="Contoh: Berhenti istirahat 20 menit"
                />
              </div>
              {message && (
